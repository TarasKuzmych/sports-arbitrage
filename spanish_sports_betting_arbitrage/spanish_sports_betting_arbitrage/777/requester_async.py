...
async def main():
    ...
    original_links = await process_links(ROOT_DIR / '777/777_links.txt', ROOT_DIR / '777/777_links.html', base_url, excluded_words)
    ...
    new_unique_links = await extract_new_unique_links(ROOT_DIR / '777/777_links.html', base_url, excluded_words, set(original_links))
        ...
        # Save new unique links to '777_v2.txt' with condition 10 > num_segments >= 3
    await save_content_to_file(ROOT_DIR / '777/777_links_v2.txt', '\n'.join(new_unique_links), 'w')

    # Process final links and append content to '777.html'
    await process_final_links(ROOT_DIR / '777/777_links_v2.txt', ROOT_DIR / '777/777.html')
