...
async def extract_and_save_links(html_content, base_url, file_path, excluded_words, segment_condition=(2, 1)):
    ...
    for link in links:
        ...
        if not any(word in full_url for word in excluded_words):
            num_segments = len(full_url.split('/')) - 3  # Adjust based on your URL structure; subtract the base URL segments
...
async def extract_new_unique_links(processed_file_path, base_url, excluded_words, original_links):
    ...
    try:
        ...
        for page_link in page_links:
            ...
            if not any(word in full_url for word in excluded_words) and num_segments >= 3:
...
async def main():
    ...
    html_content = await fetch_and_process_url(url, random.randint(3, 6), random.uniform(0.25, 0.75))
    ...
    original_links = await process_links(ROOT_DIR / '1xbet/1xbet_links.txt', ROOT_DIR / '1xbet/1xbet_links.html', base_url, excluded_words)
    ...
    await process_final_links(ROOT_DIR / '1xbet/1xbet_links_v2.txt', ROOT_DIR / '1xbet/1xbet.html')
